...
2. 验证输入 Markdown 格式是否符合 `resume-tailor/references/resume-standards.md` 的格式规范。
3. 读取 `references/export-formats.md` 确定格式、命名、排版和失败兜底；读取 `references/layout-quality-constraints.md` 控制字号、行高、留白、照片位和内容密度；需要参考图风格时读取 `references/visual-template-guide.md`。
4. 生成完整简历正文，不只输出后续材料片段。
5. 优先生成可打开的 `resume.html` 供即时预览；HTML 页面必须包含”编辑””重置””上传照片”和”导出 PDF”按钮，支持页面内全部简历文字实时编辑、上传真实照片后再打印导出。
6. 输出 `ResumeDocument`，包含文件路径、格式说明、生成时间和待确认项。
...
- `assets/templates/resume.md`：内部草稿模板
