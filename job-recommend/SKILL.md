...
5. 输出 `JobRecommendation` 表格，初始状态为"待用户确认"。
6. **必须导出 Excel 清单**：使用 `scripts/export_job_recommendations.py` 导出 `.xlsx` 文件到 `简历材料/` 目录，文件名格式为 `岗位推荐清单-{目标岗位}-{YYYYMMDD}.xlsx`。
7. 若网络或平台访问不可用，输出搜索关键词、筛选规则和待用户自行确认清单，不编造岗位。
8. 用户明确选择岗位并确认投递后，执行"确认后投递流程"。

### Excel 导出约束

| 约束项 | 要求 |
|--------|------|
| 必须导出 | 每次岗位推荐都必须生成 Excel 文件 |
| 保存路径 | 当前工作目录下的 `简历材料/` 文件夹 |
| 文件命名 | `岗位推荐清单-{目标岗位}-{YYYYMMDD}.xlsx` |
| 格式要求 | 首行冻结、启用筛选、列宽适配、链接可点击 |

**只有在搜索工具完全不可用时**，才可跳过 Excel 导出，改为输出 Markdown 清单模板。
...
**必须执行真实搜索：**
1. 优先使用 autoglm-websearch 进行快速搜索
2. 从搜索结果中提取真实岗位信息
3. 验证岗位详情页链接可访问
